  ...
  return (
    ...
    <div className={`flex flex-col gap-4 fixed w-full z-20 bg-black text-white`}>
      <div className={`container mx-auto h-auto lg:max-w-[1070px] px-5 flex lg:justify-between justify-between items-center transition-all`}>
        {/* Search button wrapper always present to keep layout */}
        <div className='hidden lg:block size-6'>
          {!pathname.includes('/checkout') && (
            <div onClick={setSearchModalOpen} className='cursor-pointer'>
              <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <path d="M16.4883 16.491L21.25 21.25" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </div>
          )}
        </div>
        ...
        <div className='flex items-center gap-5'>
          ...
          {/* Cart button hidden on /checkout */}
          <div onClick={() => {setNavModalClose(); setCartOpen()}} className={`${pathname.includes('/checkout') ? 'hidden' : 'block'} relative size-6 lg:size-6 cursor-pointer`}>
            <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
