  ...
  return (
    <div className="w-full bg-white pt-10 h-auto z-50">
      <div className="text-black flex flex-col container mx-auto max-w-[1070px] px-5">
        <h1 className="lg:text-3xl text-4xl font-bold">Featured</h1>
        <div className='relative grid grid-cols-2 lg:gap-10 gap-5 lg:py-10 py-10'>
          {catalog?.map((featured) => {
            ...
            return(
            <>
            <div className='flex flex-col' key={featured.id}>
              <Link href={`/catalog/${featured.title}`} data-aos="fade-up" data-aos-duration='4000' data-aos-once={true} className='lg:hidden flex flex-col h-max w-full group lg:rounded-2xl rounded-lg cursor-pointer duration-400 transition-all'>
                <div className='h-full w-full rounded-t-lg overflow-hidden duration-500'>
                  <CldImage src={featured.image} width="200" height="500" alt="tee" className='w-full h-[250px] ease-in-out transition-all duration-500' loading='lazy'/>
                </div>
              ...
              <Link href={`/catalog/${featured.title}`} data-aos="fade-up" data-aos-duration='3000' data-aos-once={true} onMouseEnter={() => setHoveredId(featured.id)} onMouseLeave={() => setHoveredId(null)} key={featured.id} className='relative lg:flex flex-col hidden h-max w-full group hover:shadow-2xl lg:rounded-2xl rounded-lg cursor-pointer duration-400 transition-all'>
