      ...
      <div className="text-black flex flex-col container mx-auto max-w-[1070px] px-5">
        ...
        <div className='relative grid grid-cols-2 lg:gap-10 gap-5 lg:py-10 py-10'>
          {featuredData.map((featured) => {
            const stockEntry = stocks.find((stock) => stock.catalogTitle === featured.title);
            const isOutOfStock = 
            ...
            <div className='relative flex flex-col'>
              <div onClick={() => seeProduct(featured.title)} data-aos="fade-up" className='lg:hidden h-max w-full group hover:shadow-2xl lg:rounded-2xl rounded-lg cursor-pointer duration-400 transition-all'>
              ...
              <div onClick={() => seeProduct(featured.title)} onMouseEnter={() => setHoveredId(featured.id)} onMouseLeave={() => setHoveredId(null)} key={featured.id} className='lg:block hidden h-max w-full group hover:shadow-2xl lg:rounded-2xl rounded-lg cursor-pointer duration-400 transition-all'>
                <div className='h-full[400px] w-full lg:rounded-t-2xl rounded-t-lg overflow-hidden duration-500'>
              ...
              <div className='absolute top-5 left-5'>
                {isOutOfStock 
                ? 
                  <span className='bg-black py-1 px-4 rounded-full text-white'>Out of Stock</span> 
                : 
                  ''  
                }
              </div>
